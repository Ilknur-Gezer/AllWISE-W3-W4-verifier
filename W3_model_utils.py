...
def train_models():
    df = pd.read_csv(W3_config.TRAINING_DATA)
    X = df.iloc[:, :-1]
    y = df.iloc[:, -1]
    ...
    # Save the best model
    os.makedirs(W3_config.MODEL_DIR, exist_ok=True)
    best_model = models[best_model_key]
    best_model_path = os.path.join(W3_config.MODEL_DIR, f"model_{best_model_key}.pkl")
    joblib.dump(best_model, best_model_path)

    # Save the path to best model
    with open(W3_config.BEST_MODEL_PATH_FILE, "w") as f:
...
def predict_data():
    ...
    with open(W3_config.BEST_MODEL_PATH_FILE, "r") as f:
        best_model_path = f.read().strip()
    model = joblib.load(best_model_path)

    df = pd.read_csv(W3_config.PREDICTION_DATA)
    ...
    pred_class[y_prob <= W3_config.THRESHOLD] = 'fake'
    pred_class[y_prob > W3_config.THRESHOLD] = 'real'

    result = pd.concat([df.iloc[:, 0], pred_class, pd.Series(y_prob)], axis=1)
    result.columns = ['AllWISE', 'class_W3', 'Prob_W3_real']
    result = result.round(4)
    result.to_csv("/Users/igezer/Desktop/allwise_cleaning/AllWISE_OrionFOV_W3_classification_result.csv", index=False)
    print("Predictions saved to: AllWISE_OrionFOV_W3_classification_result.csv")
